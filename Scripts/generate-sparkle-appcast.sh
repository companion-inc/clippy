...
archive_path="${1:-$repo_root/Clippy-macOS.zip}"
output_path="${2:-$repo_root/appcast.xml}"
sparkle_version="${SPARKLE_VERSION:-2.9.3}"
release_tag="${CLIPPY_RELEASE_TAG:-${GITHUB_REF_NAME:-}}"

if [ -z "$release_tag" ]; then
  release_tag="$(git -C "$repo_root" describe --tags --exact-match 2>/dev/null || true)"
fi
if [ -z "$release_tag" ]; then
  echo "ERROR: CLIPPY_RELEASE_TAG or an exact git tag is required for appcast URLs." >&2
  exit 1
fi
...
download_prefix="${CLIPPY_DOWNLOAD_URL_PREFIX:-https://github.com/companion-inc/clippy/releases/download/$release_tag/}"
product_link="${CLIPPY_PRODUCT_LINK:-https://github.com/companion-inc/clippy/releases/latest}"
