...
/// Shared MS Agent-style balloon drawing. Clippy's body is raster, but the
/// original Office Assistant balloon itself uses rounded corners and a smooth
/// single outline.
public enum ClippyBalloonStyle {
    public static func font(_ size: CGFloat, bold: Bool = false, spec: ClippyBalloonSpec = .current) -> NSFont {
    ...
    public static func makeShapeLayer(spec: ClippyBalloonSpec = .current) -> CAShapeLayer {
    ...
    public static func path(size: CGSize, spec: ClippyBalloonSpec = .current) -> CGPath {
