...
public struct SidekickInvocationSuggestion: Equatable, Sendable {
...
public struct SidekickInvocationRecommendation: Equatable, Sendable {
    public let message: String
    public let suggestions: [SidekickInvocationSuggestion]

    public init(message: String, suggestions: [SidekickInvocationSuggestion]) {
        self.message = message
        self.suggestions = suggestions
    }
}

public enum SidekickInvocationSuggestions {
    ...
    public static let recommendationSchema = AgentOutputSchema(jsonObject: [
        ...
        "properties": [
            "message": [
                "type": "string",
                "description": "Short Sidekick speech-bubble line above the options. Acknowledge the likely situation or intent; do not name the app generically.",
                "minLength": 1,
                "maxLength": 140,
            ],
            "options": [
                ...
                "items": [
                    ...
                    "properties": [
                        ...
                        "prompt": [
                            "type": "string",
                            "description": "Exact instruction Sidekick should run if the user picks this option.",
                            "minLength": 1,
                        ],
                    ],
                ],
    ...
    public static func parseRecommendation(from text: String, limit: Int = 3) -> SidekickInvocationRecommendation? {
        ...
        var seen = Set<String>()
        let suggestions: [SidekickInvocationSuggestion] = options.compactMap { dto -> SidekickInvocationSuggestion? in
            ...
            return SidekickInvocationSuggestion(title: title, prompt: prompt)
        }
        let limited = Array(suggestions.prefix(limit))

        guard limited.isEmpty == false else { return nil }
        return SidekickInvocationRecommendation(message: message, suggestions: limited)
    }
