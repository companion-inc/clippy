...
public struct ClippyResourceManifest: Codable, Equatable, Sendable {
...
public enum CharacterResourceLoader {
    ...
    public static func loadManifest(from root: URL) throws -> ClippyResourceManifest {
        let data = try Data(contentsOf: root.appending(path: "manifest.json"))
        return try JSONDecoder().decode(ClippyResourceManifest.self, from: data)
    }

    public static func clippyPackDescriptor(from root: URL) throws -> CharacterPack {
