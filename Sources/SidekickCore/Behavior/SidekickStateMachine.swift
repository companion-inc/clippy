...
public enum SidekickRuntimeEvent: Equatable, Sendable {
    case commandStarted(SidekickCommand)
    case commandFinished(SidekickCommand, SidekickRequestStatus)
...
public struct SidekickStateSnapshot: Equatable, Sendable {
    public let state: SidekickState
...
public struct SidekickStateMachine: Sendable {
    private var snapshot: SidekickStateSnapshot

    public init(initialState: SidekickState = .hidden) {
        self.snapshot = SidekickStateSnapshot(state: initialState)
    }

    public var current: SidekickStateSnapshot {
        snapshot
    }

    @discardableResult
    public mutating func apply(_ event: SidekickRuntimeEvent) -> SidekickStateSnapshot {
        switch event {
        ...
        case let .voice(event):
            snapshot = snapshot.replacing(
                state: VoiceEventRouter.sidekickState(for: event) ?? snapshot.state,
    ...
    private func state(for command: SidekickCommand) -> SidekickState {
        switch command {
        case .show:
            return .showing
        case .hide:
    ...
    private func stateAfter(command: SidekickCommand, status: SidekickRequestStatus) -> SidekickState {
    ...
    private func state(forToolName name: String) -> SidekickState {
...
private extension SidekickStateSnapshot {
    func replacing(
        state: SidekickState,
        message: String?,
        approval: ApprovalRequest?
    ) -> SidekickStateSnapshot {
        SidekickStateSnapshot(state: state, lastMessage: message, awaitingApproval: approval)
    }
}
