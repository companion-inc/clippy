...
/// app tile across into the list. Rendered here in Sidekick's Win95 / Office-97 skin.
@MainActor
...
/// Sidekick tile, and an "Open System Settings" default button.
final class RetroPermissionPanel: NSView {
    ...
    init(
        ...
        let titleBar = RetroTitleBar(title: "Sidekick — \(permissionName) Access")
        ...
        let blurbText = allowsDragging
            ? "Add Sidekick to the \(permissionName) list so it can do its job. Drag the Sidekick tile below into the list — or click Open System Settings and switch Sidekick on."
            : "Add Sidekick to the \(permissionName) list so it can do its job. Click Open System Settings and switch Sidekick on, then click Done."
        ...
        let tileLabel = allowsDragging
            ? "Drag Sidekick into \(permissionName) settings"
            : "Turn Sidekick on in \(permissionName) settings"
...
/// The draggable Sidekick tile: the app's own icon + label inside a raised chip, carrying
...
final class RetroDragTile: NSView, NSDraggingSource {
    ...
    init(appURL: URL, label: String, allowsDragging: Bool = true) {
        ...
        icon.image = Self.sidekickCharacterImage(for: appURL)
    ...
    override func mouseDown(with event: NSEvent) {
        ...
        let icon = Self.sidekickCharacterImage(for: appURL)
    ...
    /// Draw the real Sidekick character in the draggable tile, while the drag payload
    /// remains the signed .app bundle that macOS accepts in privacy lists.
    private static func sidekickCharacterImage(for appURL: URL) -> NSImage {
