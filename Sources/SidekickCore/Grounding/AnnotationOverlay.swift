...
public enum AnnotationMark: Equatable, Sendable {
    ...
    /// `POINT` gets a small precision dot; Sidekick's own body remains the pointer.
...
/// Borderless, transparent, click-through overlay that draws Sidekick's on-screen marks
/// (point dots, target/hover rings, highlight outlines, shape paths) in global screen coordinates.
/// Sidekick's body is the visible pointer; this overlay provides precise Sidekick-style ink.
@MainActor
public final class AnnotationOverlayWindow {
    ...
    /// Draw marks as Sidekick visual beats: shape paths reveal over time, and
    ...
    /// Draw a spatial scene as Sidekick visual beats.
...
public struct UserScreenAnnotation: Equatable, Sendable {
    ...
    public var scene: DrawingScene {
        DrawingScene(marks: marks, anchor: anchor)
    }

    public func promptBlock(for screenshots: [ScreenPerception.Screenshot]) -> String? {
        guard !strokes.isEmpty else { return nil }
        let resolvedStrokes = currentStrokes()
        let screenshot = screenshot(for: resolvedStrokes, in: screenshots)
            ?? screenshots.first { $0.screenIndex == screenIndex }
        guard let screenshot else {
            return fallbackPromptBlock(strokes: resolvedStrokes)
        }
        let lines = resolvedStrokes.enumerated().map { index, stroke -> String in
        ...
        The user drew \(resolvedStrokes.count) freehand stroke(s) before this question. Treat these as user-authored yellow ink over the current screenshot, used to point out what they mean by "this", "that", or "what do you think". Do not call them Sidekick's own marks.
        \(lines.joined(separator: "\n"))
        """
    }

    public func currentStrokes() -> [[CGPoint]] {
        scene.resolvedPathPoints(windowFrameProvider: { $0.currentFrame() ?? $0.initialFrame })
    }

    private func screenshot(
        for resolvedStrokes: [[CGPoint]],
        in screenshots: [ScreenPerception.Screenshot]
    ) -> ScreenPerception.Screenshot? {
        if case let .window(window) = anchor {
            let frame = window.currentFrame() ?? window.initialFrame
            return screenshots.max { lhs, rhs in
                lhs.screenFrame.intersection(frame).area < rhs.screenFrame.intersection(frame).area
            }
        }
        if let firstPoint = resolvedStrokes.first?.first {
            return screenshots.first { $0.screenFrame.contains(firstPoint) }
        }
        return screenshots.first { $0.screenIndex == screenIndex && $0.screenFrame == screenFrame }
    }

    private func fallbackPromptBlock(strokes: [[CGPoint]]) -> String {
...
public final class UserAnnotationController {
    ...
    private var anchor: DrawingAnchor = .screen
    ...
    public func begin(
        ...
        self.anchor = anchor
        strokes = annotation?.screenFrame == selectedScreen.frame && annotation?.anchor == anchor
            ? annotation?.currentStrokes() ?? []
            : []
        currentStroke = []
    ...
    public func finish() -> UserScreenAnnotation? {
        ...
        return UserScreenAnnotation(screenIndex: screenIndex, screenFrame: screen.frame, strokes: strokes, anchor: anchor)
    }
    ...
    private func commitCurrentStroke() {
        let simplified = UserScreenAnnotation(screenIndex: screenIndex, screenFrame: screen?.frame ?? .zero, strokes: [currentStroke], anchor: .screen).strokes.first
        if let simplified {
...
private extension CGRect {
    var area: CGFloat {
        guard !isNull, !isEmpty, !isInfinite, width > 0, height > 0 else { return 0 }
        return width * height
    }
}
