...
public struct DrawingScene: Equatable, Sendable {
    ...
    public func resolvedPathPoints(windowFrameProvider: (DrawingWindowAnchor) -> CGRect?) -> [[CGPoint]] {
        guard let frame = resolvedFrame(windowFrameProvider: windowFrameProvider) else { return [] }
        return objects.compactMap { $0.pathPoints(in: frame) }
    }
...
public struct DrawingObject: Equatable, Sendable {
    ...
    public func pathPoints(in referenceFrame: CGRect) -> [CGPoint]? {
        geometry.pathPoints(in: referenceFrame)
    }
