...
/// Sidekick renders these by moving and gesturing with
...
///   [TARGET:x,y,r:label]     one click/commit Sidekick can observe + recapture from
...
public enum GroundingTag: Equatable, Sendable {
    ...
    /// Sidekick performs one of its own animations to express what it's doing or feeling.
    ...
    /// The label Sidekick shows in its speech bubble for this directive.
    ...
    /// The primary on-screen anchor Sidekick should point at (first point for shapes).
    ...
    /// Whether this directive is a Swift-observable action Sidekick should recapture after.
    ...
    /// Sidekick's body animation, but it is not a screen grounding mark.
    ...
    /// and Sidekick's body land where the model actually meant. Radii scale with the
...
/// An assistant reply split into the text Sidekick speaks and the directives it acts on.
...
/// Parses Sidekick grounding tags out of an assistant reply.
