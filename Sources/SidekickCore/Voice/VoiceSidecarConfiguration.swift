...
public extension VoiceSidecarConfiguration {
    ...
    static var irisVoiceSidecar: VoiceSidecarConfiguration {
        ...
        return VoiceSidecarConfiguration(
            executablePath: environment["SIDEKICK_VOICE_SIDECAR_EXECUTABLE"] ?? environment["CLIPPY_VOICE_SIDECAR_EXECUTABLE"] ?? "uv",
            workingDirectoryPath: environment["SIDEKICK_IRIS_VOICE_DIR"] ?? environment["CLIPPY_IRIS_VOICE_DIR"] ?? defaultIrisVoiceDirectoryPath
        )
    }
