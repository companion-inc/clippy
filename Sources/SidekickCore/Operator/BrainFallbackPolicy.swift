...
public struct BrainFallbackOffer: Equatable, Sendable {
    ...
    public let toModel: SidekickModel
    ...
    public var prompt: String {
        ...
        case .authentication:
            return "\(fromProviderName) sign-in expired. Switch to \(toProviderName)?"
        case .connection:
    ...
    public var keepTitle: String {
        if reason == .connection || reason == .authentication {
...
public enum BrainFallbackPolicy {
    ...
    public static func offer(
        ...
        guard let issue = SidekickUserFacingError.providerIssue(for: text) else {
            return nil
        }
        let reason: BrainFallbackOffer.Reason
        switch issue {
        case .usageLimit:
            reason = .usageLimit
        case .authentication:
