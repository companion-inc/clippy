...
/// Sidekick's brain, running locally: drives the installed `claude` CLI in print
/// mode. The first turn opens a session; every later turn resumes it, so Claude
/// Code itself holds the full back-and-forth history, runs the agent loop, and
/// executes tools on this Mac. No API key — uses the user's local Claude auth.
public actor ClaudeCLIConversation {
    ...
    /// Best-effort discovery of the local `claude` binary.
