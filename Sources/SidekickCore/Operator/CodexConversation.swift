...
/// Sidekick's Codex brain uses the same app-server surface that rich Codex
...
/// chunks while one thread stays alive across Sidekick turns.
public actor CodexConversation: StructuredOutputAgentBrain {
    ...
    private static let diagnosticsQueue = DispatchQueue(label: "ai.companion.sidekick.codex.diagnostics")
    ...
    public init(
        ...
        recordReplayRuntime: MCPServerRuntime? = SidekickRecordReplayMCPConfig.defaultRuntime(),
        ...
        self.mcpRuntimes = Self.uniqueRuntimes([computerUseRuntime, annotationRuntime, recordReplayRuntime].compactMap { $0 })
    ...
    public static func locateBinary() -> String? {
        if let managed = SidekickRuntimeLocator.codexExecutablePath() {
    ...
    private func runStreaming(
        ...
        do {
            ...
            continuation.yield(.status(hasOpenThread ? "Using the open thread" : "Opening the Sidekick thread"))
    ...
    private func ensureAppServer(process box: ProcessBox) throws -> AppServerConnection {
        ...
        process.environment = SidekickSecrets.environmentByAddingLocalAPIKeys()
    ...
    private static func appendDiagnostic(_ text: String, to logURL: URL?) {
        ...
        appendDiagnosticData(Data(text.utf8), label: "sidekick", to: logURL)
    }
