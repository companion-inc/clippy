...
/// Sidekick can default to the right brain out of the box instead of assuming one.
///
...
/// "signed in": Sidekick runs on the user's subscription, so a key-only setup should
/// still walk the user through signing in. We check files, not the Keychain, so
/// detection never pops a "Sidekick wants to use a credential" prompt.
public enum BrainDiscovery {
    public struct Status: Equatable, Sendable {
        public let backend: SidekickModel.Backend
    ...
    /// is present. If both are signed in, prefer Codex because Sidekick wires Cua
    /// through the Codex app-server path.
    public static func defaultModel() -> SidekickModel {
