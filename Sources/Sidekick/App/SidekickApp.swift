...
final class SidekickApp: NSObject, NSApplicationDelegate {
    ...
    private let clippySessionID = "local"
    ...
    private static func makeMascot() -> any DesktopMascot {
        ...
        if let clippy = try? ClippyMascot(packRoot: clippyResourceRoot(), scale: characterScale) {
            return clippy
        }
        return MorphMascot()
    }
    ...
    private func startMascot(_ mascot: any DesktopMascot) {
        let bubble = SidekickBubbleController(theme: mascot.theme)
    ...
    private func sendMessage(_ text: String) {
        ...
        log("user: \(text)")
        applyAgentActivity(agentID: .clippy, sessionID: clippySessionID, event: .userPromptSubmit)
    ...
    private func receiveReply(_ turn: ClaudeCLIConversation.Turn) {
        ...
        log("sidekick: \(turn.text.prefix(120))")
        applyAgentActivity(agentID: .clippy, sessionID: clippySessionID, event: turn.isError ? .stopFailure : .stop)
    ...
    private func applyAgentActivity(
        ...
        let visibleState = agentSessions.apply(AgentActivityEvent(
            agentID: agentID,
            sessionID: sessionID.isEmpty ? "default" : sessionID,
            lifecycleEvent: event
        ))
        log("agent: \(agentID.rawValue) \(sessionID) \(event.rawValue) -> \(visibleState.rawValue)")
        playActivityState(visibleState)
    }
    ...
    private func makeContextMenu() -> NSMenu {
        ...
        let quit = NSMenuItem(title: "Quit Sidekick", action: #selector(quitSidekick), keyEquivalent: "")
        quit.target = self
        menu.addItem(quit)
    ...
    private func applyAgentCommand(_ command: String) {
        let parts = command.split(separator: ":", maxSplits: 3, omittingEmptySubsequences: false)
        guard parts.count == 4,
              let agentID = SidekickAgentID(rawValue: String(parts[1])),
              let event = AgentLifecycleEvent(rawEvent: String(parts[3]))
        else {
            return
        }
        applyAgentActivity(agentID: agentID, sessionID: String(parts[2]), event: event)
    }
